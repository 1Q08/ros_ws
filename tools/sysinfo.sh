...
install_pkg() {
    ...
    if ! sudo apt-get update; then
        echo -e "\n${YELLOW}⚠️apt-get update 出现警告${NC}"
        if ! ask_yes_no "是否使用现有缓存继续安装? (建议优先检查软件源配置)"; then
            echo -e "\n${YELLOW}已取消安装, 请检查软件源配置后重试${NC}"
            return 1
        fi
    fi
...
if ! sudo -v; then
    echo -e "\n${RED}❗错误: 当前用户无 sudo 权限，无法执行安装操作${NC}"
    exit 1
fi
...
while true; do
    ...
    case "${choice// /}" in  # 去除空格后匹配，防止误输空格
        1)
            if [ -z "$local_ros_distro" ]; then  # 检测环境变量是否为空，local_ros_distro 输出 ROS 发行版名称
                echo -e "\n${RED}❗错误: 未检测到 ROS 环境, 无法安装 rqt${NC}"
            else
                echo -e "\n${YELLOW}检测到 ROS 发行版: ${local_ros_distro}${NC}"
        ...
        2)
            if [ -z "$local_ros_distro" ]; then  # 未检测到 ROS 环境则无法确定发行版包名
                echo -e "\n${RED}❗错误: 未检测到 ROS 环境, 无法安装 plotjuggler${NC}"
            else
                echo -e "\n${YELLOW}检测到 ROS 发行版: ${local_ros_distro}${NC}"
            ...
            ;;
        3)
            echo -e "\n${YELLOW}正在使用第三方 PPA 安装 ubuntu-cleaner ...${NC}"
            ...
            if sudo add-apt-repository -y -n ppa:gerardpuig/ppa; then
                ...
                echo -e "\n${RED}❗添加 PPA 失败, 无法安装 ubuntu-cleaner${NC}"
            fi
            ;;
        4)
        ...
        5)
            ...
            if ! sudo apt-get update; then
                echo -e "\n${YELLOW}⚠️部分软件源刷新失败${NC}"
                if ! ask_yes_no "是否基于现有缓存继续升级? (建议优先检查软件源配置)"; then
                    echo -e "\n${YELLOW}已取消升级, 请检查软件源配置后重试${NC}"
                    echo -e "${BLUE}------------------------------------------------${NC}"
                    continue
                fi
            fi
            if sudo apt-get upgrade -y; then
                ...
                echo -e "\n${RED}❗系统更新失败, 请检查网络或软件源${NC}"
            fi

            echo -e "\n${YELLOW}预览 autoremove 结果如下:${NC}"
        ...
        6)
            if [ -z "${DISPLAY:-}" ]; then  # 检查 $DISPLAY 变量是否为空，若为空则说明没有图形界面
                echo -e "\n${RED}❗未检测到图形界面 (DISPLAY 变量未设置)${NC}"
                echo -e "\n${RED}无法自动打开浏览器，请手动访问以下链接下载 VOFA+: ${GREEN}https://www.vofa.plus/${NC}"
                read -rp "按 Enter 键继续..." _
            elif ! command -v xdg-open &>/dev/null; then  # 图形界面存在但缺少 xdg-open
                echo -e "\n${YELLOW}未找到 xdg-open, 请手动访问: ${GREEN}https://www.vofa.plus/${NC}"
                read -rp "按 Enter 键继续..." _
            else
                echo -e "\n${YELLOW}正在使用默认浏览器打开 VOFA+ 串口助手官网 ...${NC}"
                xdg-open https://www.vofa.plus/ 2>/dev/null &  # 忽略错误输出并且立马运行下一条命令
                echo -e "\n${GREEN}浏览器已打开, 请自行选择正确版本的 VOFA+ 串口助手下载安装${NC}"
                read -rp "按 Enter 键继续..." _
            fi
            ;;
        0)
